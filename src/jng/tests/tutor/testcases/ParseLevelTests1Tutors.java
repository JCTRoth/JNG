...
public class ParseLevelTests1Tutors {
	...
	public void parsePlanesTest() {
		...
		assertEquals(673.0f, minimal.getEnemyPlanePosition(1).y, accuracy);
		assertEquals(1420.0f, minimal.getEnemyPlanePosition(2).x, accuracy);
		assertEquals(527.0f, minimal.getEnemyPlanePosition(2).y, accuracy);
		...
		assertEquals("pathY", 350.0f, path.get(1).y, accuracy);
		
		path = minimal.getEnemyPlaneMovementPath(1);
		assertEquals("pathX", 1420.0f, path.get(0).x, accuracy);
		assertEquals("pathY", 673.0f, path.get(0).y, accuracy);
		assertEquals("pathX", 1000.0f, path.get(1).x, accuracy);
		assertEquals("pathY", 673.0f, path.get(1).y, accuracy);
		
		path = minimal.getEnemyPlaneMovementPath(2);
		assertEquals("pathX", 1420.0f, path.get(0).x, accuracy);
		assertEquals("pathY", 527.0f, path.get(0).y, accuracy);
		assertEquals("pathX", 1000.0f, path.get(1).x, accuracy);
		assertEquals("pathY", 527.0f, path.get(1).y, accuracy);
		
		for (int i = 0; i < 3; ++i)
		{
			assertEquals("plane3", minimal.getEnemyPlaneEntityID(i));
